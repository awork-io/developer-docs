...
The awork API supports [PKCE (Proof Key for Code Exchange)](https://oauth.net/2/pkce/) for enhanced security in OAuth 2.0 flows. PKCE is especially recommended for mobile apps and single-page applications that cannot securely store client secrets.

#### PKCE Parameters

When using PKCE, include these additional parameters in your authorization request:

* `code_challenge`: Base64-URL encoded SHA256 hash of the `code_verifier`
* `code_challenge_method`: Use `S256` (recommended) or `plain`
...
**Note:** For public clients using PKCE, the `client_secret` is not required.

For detailed implementation guidance, see the [OAuth 2.0 PKCE specification](https://oauth.net/2/pkce/).
