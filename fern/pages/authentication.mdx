...
PKCE protects the authorization code flow by binding the authorization request to the token request. Generate a unique `code_verifier` for each authorization attempt, send its SHA-256 challenge as `code_challenge` in the authorization request, and send the original `code_verifier` when exchanging the authorization code for tokens.

Public clients such as web apps, mobile apps, CLI tools, and MCP clients should use PKCE because they cannot safely store a client secret.
