...
For example, if you want to get 50 users, you would end up making a request with the following url: `/users?page=1&pageSize=50`.\
When you make a request to retrieve the users from the API, you get the response as well as its headers. The headers of the response contain a property named `aw-totalitems`, which provides the information about the maximum retrievable results.
