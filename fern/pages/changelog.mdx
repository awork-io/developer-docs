...
<CardGroup cols={2}>
  <Card title="Recommended path" icon="circle-check">
    Use project-scoped endpoints (`GET /projects/{projectId}/projectstatuses` and `GET /projects/{projectId}/taskstatuses`).
  </Card>
  <Card title="Alternative path" icon="code-branch">
    Keep using global `GET /projectstatuses` and filter by `workflowId` or `projectId`.
  </Card>
</CardGroup>

<Check title="Recommended" icon="circle-check">
Use this migration path by default.
</Check>
...
#### Alternative migration path (keep global `GET /projectstatuses`)

If you need to keep using the global endpoint, as many integrations currently do:
- Fetch the project first and read `workflowId`.
- If `workflowId` is set, filter statuses by `workflowId`.
- If `workflowId` is null, filter statuses by `projectId`.

```sh title="1) Fetch project and read workflowId"
curl -X GET "https://api.awork.com/api/v1/projects/3e664818-9503-4ec8-9f1f-35d5ad7a57ad" \
  -H "Authorization: Bearer {token}"
```

```json title="Project response excerpt"
{
...
```sh title="2) Fetch all project statuses"
